...
# 阶段4：分组与起始列
START_COL_1_BASED = 4  # 从第4列起求平均（1基）
...
def extract_k1(val):
    # 支持三种形式：dict、标准 JSON 字符串、单引号 Python 字面量
...
def ensure_k1_last(df: pd.DataFrame) -> pd.DataFrame:
    # 若存在 pass_params 列则提取 k1，放到最后一列；否则不动
...
def run_stage2_score_df(df_stage1: pd.DataFrame) -> pd.DataFrame:
    ...
    # 确保 k1 在最后一列（如果已有则移动，如果没有则从 pass_params 提取）
...
def run_stage4_average_df(df_stage3: pd.DataFrame) -> pd.DataFrame:
    ...
    # 确保 k1 在最后一列（若 cols_to_avg 中包含 k1 则已是聚合后的 k1 均值）
    result = ensure_k1_last(result)  # 若聚合结果里没有 k1（极端情况），此行可从 pass_params 提取；一般不会存在 pass_params 列
...
def main():
    ...
    # Sheet1：原始输入 + k1（最后一列）
    df_sheet1 = ensure_k1_last(df_input_full.copy())
