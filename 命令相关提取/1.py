...
WEIGHTS = {
    "command_correctness": 0.3,
    "security_protection": 0.25,
    "environment_adaptation": 0.2,
    "portability": 0.15,
    "output_readability": 0.1,
}
...
def extract_first_json_object(text: str) -> Optional[str]:
    ...
    if i >= n:
        return None
    start = None; depth = 0; in_string = False; string_quote = ""; escape = False
    for idx in range(i, n):
        ...
        if in_string:
            ...
            if ch in ('"', "'"):
                in_string = True; string_quote = ch
            elif ch == "{":
                if depth == 0:
                    start = idx
                depth += 1
...
def json_first_try(text: str) -> Optional[Dict]:
    ...
    if j:
        candidates += [j,
                       cleanup_json_trailing_commas(j),
                       strip_wrapping_double_braces(j),
                       cleanup_json_trailing_commas(strip_wrapping_double_braces(j))]
    candidates += [text,
                   cleanup_json_trailing_commas(text),
                   strip_wrapping_double_braces(text),
                   cleanup_json_trailing_commas(strip_wrapping_double_braces(text))]
    for s in candidates:
        try:
            return json.loads(s)
        except Exception:
...
def balanced_block_from(text: str, start_brace_idx: int) -> Optional[str]:
    ...
    if start_brace_idx < 0 or start_brace_idx >= n or text[start_brace_idx] != "{":
        return None
    depth = 0; in_string = False; string_quote = ""; escape = False; start = start_brace_idx
...
def run_stage2_score_df(df_stage1: pd.DataFrame) -> pd.DataFrame:
    ...
    scores_df.columns = [f"{k}-score" for k in scores_df.columns]
...
def compute_weighted_score(row, score_cols_map):
    total = 0.0
    for base, col in score_cols_map.items():
        val = row.get(col, None)
        try:
            num = float(val)
        except (TypeError, ValueError):
            num = 0.0
        total += num * WEIGHTS[base]
    return total

def run_stage3_weight_df(df_stage2: pd.DataFrame) -> pd.DataFrame:
    print("阶段3：计算加权分")
    score_cols_map = {}
    missing = []
    for base in WEIGHTS.keys():
        col_name = f"{base}-score"
        if col_name in df_stage2.columns:
            score_cols_map[base] = col_name
        else:
            missing.append(col_name)
    if missing:
        print("警告：以下加权所需列未找到，将按 0 计入：")
        for m in missing:
            print(f"  - {m}")
    if not score_cols_map:
        print("错误：未找到任何匹配的 '*-score' 列，无法计算加权结果。")
        sys.exit(1)
    df_out = df_stage2.copy()
    df_out[OUTPUT_COL] = df_out.apply(lambda row: compute_weighted_score(row, score_cols_map), axis=1)
    df_out = ensure_k1_last(df_out)
    df_out = move_column_to_end(df_out, "k1")
    return df_out

def run_stage4_average_df(df_stage3: pd.DataFrame) -> pd.DataFrame:
    print("阶段4：分组平均计算")
    if df_stage3.shape[1] < START_COL_1_BASED:
        print(f"列数不足。需要至少 {START_COL_1_BASED} 列，当前只有 {df_stage3.shape[1]} 列。")
        sys.exit(1)
    if PREFER_ID_NAME in df_stage3.columns:
        id_col = PREFER_ID_NAME
    else:
        id_col = df_stage3.columns[0]
        if id_col != PREFER_ID_NAME:
            print(f"未找到列名“{PREFER_ID_NAME}”，将使用第1列“{id_col}”作为分组列。")
    cols_to_avg: List[str] = list(df_stage3.columns[START_COL_1_BASED - 1:])
...
def main():
    print("开始执行完整流程：排序 -> 评分提取 -> 加权 -> 分组平均（写入同一个Excel的多个Sheet）")
    try:
    ...
    df_stage3 = run_stage3_weight_df(df_stage2)
    df_stage4 = run_stage4_average_df(df_stage3)
    ...
    try:
        with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
            ...
            df_stage4.to_excel(writer, index=False, sheet_name="Sheet5")
        print(f"全部完成，已写入单一文件：{out_path}")
        print("Sheet 对应关系：")
        print("  Sheet1: 原始输入（完整列）+ k1")
        print("  Sheet2: 排序结果（prompt/response/pass_params）+ k1")
        print("  Sheet3: 评分结果（新增 *-score 列）+ k1")
        print("  Sheet4: 加权结果（新增 '加权结果' 列）+ k1")
        print("  Sheet5: 分组平均（从第4列起的数值列均值，含 k1 均值）+ k1")
    except Exception as e:
